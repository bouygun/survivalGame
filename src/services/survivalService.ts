  ...
  public simulate(parameters: survivalServisInput): string[] {
    ...
    try {
      ...
      for (const enemyPosition of enemies) {
        ...
        if (hero.hp > 0) {
          responseArr.push(
            `Hero defeated ${enemyPosition.type} with ${hero.hp} HP remaining`
          );
        } else {
          responseArr.push(
            `Hero was defeated by ${enemyPosition.type} at position ${position}`
          );
          responseArr.push(
            `Hero is Dead!! Last seen at position ${position}!!`
          );
          return responseArr;
        }
      }
